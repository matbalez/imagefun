...
const LOG_GROUP_SIZE = Number.parseInt(process.env.MDK_LIGHTNING_LOG_GROUP_SIZE ?? '', 10) || 6;
const LOG_GROUP_FLUSH_INTERVAL_MS = Number.parseInt(process.env.MDK_LIGHTNING_LOG_GROUP_INTERVAL_MS ?? '', 10) || 250;
...
export const lightningLogHandler = (entry) => {
    if (!loggingEnabled) {
        return;
    }
    lightningLogAggregator.add(entry);
};
export const lightningLogErrorHandler = (error) => {
    if (!loggingEnabled) {
        return;
    }
...
//# sourceMappingURL=lightning-logs.js.map